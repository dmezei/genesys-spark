...
describe('JSON Docs Registry - Integration Tests', () => {
    beforeAll(() => {
        ...
        if (fs.existsSync(jsonDocsPath)) {
            // File exists, just initialize registry by calling a component
            try {
                getComponent('gux-button');
            } catch (error) {
                console.warn('Error initializing registry:', error);
            }
        } else {
            console.warn('Generated json-docs.json not found. Run npm run generate-docs first.');
        }
    });
